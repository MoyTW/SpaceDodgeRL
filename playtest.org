...
+ Done for day now
+ TODO: Inventory screen re-columnizes items when you save/quit - should be consistent across games!
...
+ TODO: High turn times introduce noticable input lag, even when they shouldn't.
...
+ TODO: In autopilot readout if items = 3 & feeatures = 1, runs right off the
...
+ TODO: sometimes diagonal shots don't go where you expect - for example if you
  ...
  4-directional paths.
+ TODO: Item sprites are drawn over ship sprites whereas opposite should be true
+ TODO: You Are Here on the autopilot map
+ TODO: Draw sprites on inventory page? (sprite + name on left, description
  right maybe?)
+ TODO: You (and enemies) are willing to shoot straight into asteroids
+ TODO: Autoexplore should end at jump gate if present
+ TODO: Make it obvious that autopilot -> kill -> autoexplore is intended
  gameplay flow somehow (add "AUTOEXPLORE ZONE" button to autopilot screen?
  upper right has some empty space)
+ TODO: Your scan cursor is seriously off-center; re-work that
  - TODO: Make a full playthrough with mouse only
...
+ TODO: If you hover over a zone in one run of the autopilot menu, and then exit
  the autopilot menu, then open the menu again and move the mouse, it will crash
  with "Cannot access a disposed object". Makes sense - we redo the entire scene
  and throw away all the old stuff every time...
